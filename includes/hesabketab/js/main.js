...
function selectSubCat(maincatvalue,maincattitle){
              $.ajax({
                  type:'POST',
                  url:'includes/hesabketab/ajax/select_ajax.php',
                  data:'maincatvalue='+maincatvalue,
                  success:function(resp){
					  var data = JSON.parse(resp);
                      var option_html = "";
                      for(var i =0; i<data.length; i++){
                         option_html += '<option value="'+data[i]['tbl_coding_id']+'">'+data[i]['tbl_coding_title']+'</option>'; 
                      }
                      $('#'+maincattitle+'_sub').html(option_html);
                  }
              });
          }
