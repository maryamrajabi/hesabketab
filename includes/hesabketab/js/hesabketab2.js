function CheckMeliCode(meli_code)
{
    ...
    {
        $('#nationalcode_validation').addClass("alert-danger");
        $('#nationalcode_validation').html('طول کد ملی وارد شده باید 10 کاراکتر باشد');
        $('.next_but').attr('disabled','disabled');
         return false;       
    }
}
...
if(!Number(isbn)){
	  ...
	  $("#loading").show();
        ...
        $.ajax({
		   ...
		   swal("شابک!", "شابک باید بین 10 تا 13 رقم باشد", "warning");
		 }
...
//        alert(sumtotaldiscount);
    ...
    $(document).on('click','#add_table2',function(){
      ...
      var  approveddiscount = $('html, body').find( "input[name='approveddiscount']" ).val();		
