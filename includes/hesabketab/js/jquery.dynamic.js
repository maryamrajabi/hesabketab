    ...
    $.dynamic = function(element, options) {
        ...
        var defaults = {
            customScroll: {
                ...
                rscolor: '#fff', //color of left sidebar
            ...
            sideNav : {
                ...
                sideNavArrowIcon: 'l-arrows-left', //arrow icon for navigation
            ...
            backToTop: {
                ...
                place: 'bottom-left', //position top-right, top-left, bottom-left, bottom-right
        ...
        plugin.init = function() {
            ...
            //fixed sidebar
            if(plugin.settings.sidebar.fixed && store.get('fixed-right-sidebar') == 1) {
                this.fixedSidebar('right');
            }
            //fixed left sidebar
            if(plugin.settings.leftSidebar.fixed && store.get('fixed-left-sidebar') == 1) {
            ...
            if(plugin.settings.sidebar.rememberToggle) {
                ...
                if(store.get('sidebarToggle') == 1 && breakpoint == 'large' || store.get('sidebarToggle') == 1 && breakpoint == 'laptop') {
                    plugin.togglerightSidebar();
                    plugin.sideBarNavToggle();
                    plugin.collapseSideBarNav(false);
                    plugin.removeFixedSidebar('right');
                }
            }

            //check if left sidebar need to be toggled
            if(plugin.settings.leftSidebar.rememberToggle) {
                var breakpoint = plugin.getBreakPoint();
                if(store.get('leftSidebarToggle') == 1 && breakpoint == 'large' || store.get('leftSidebarToggle') == 1 && breakpoint == 'laptop') {
                    $('#toggle-left-sidebar').addClass('hide-left-sidebar');
                    plugin.hideleftSidebar();
                }
                if (store.get('leftSidebarToggle') == 1 && breakpoint == 'tablet' || store.get('leftSidebarToggle') == 1 && breakpoint == 'phone') {
                    $('#toggle-left-sidebar').addClass('hide-left-sidebar');
                }
            }
            ...
            $(window).load(function(){
                if (store.get('fixed-header') == 0 && store.get('fixed-left-sidebar') == 1) {
                    plugin.leftSidebarTopPosition();
                }
                plugin.stickyFooter();
            });
        ...
        plugin.fixedSidebar = function (val) {
            ...
            if (val === 'left') {
                if (breakpoint == 'large' || breakpoint == 'laptop') {
                    $('#left-sidebar').addClass('sidebar-fixed');
                    //activate slim scroll
                    $('#left-sidebar .sidebar-scrollarea').slimScroll({
                        position: "left",
                        height: '100%',
                        distance: '1px',
                        railVisible: false,
        ...
        //fix leftsidbear top position on scroll if header is not fixed but sidebar is
        plugin.leftSidebarTopPosition = function () {
            var scrollTop = $(document).scrollTop();
            if (scrollTop > 50 - 1) {
                $('#left-sidebar').addClass('rstop');
            } else {
                $('#left-sidebar').removeClass('rstop');
            }
        }
        ...
        plugin.removeFixedSidebar = function (val) {
            ...
            if (val === 'left') {
                var scrollarea = $('#left-sidebar .sidebar-scrollarea');
                $('#left-sidebar').removeClass('sidebar-fixed');
                plugin.removeScrollTo(scrollarea);
        ...
        plugin.toggleSidebar = function() {
            ...
            var toggleRButton = $('#toggle-left-sidebar');
            var breakpoint = plugin.getBreakPoint();
            ...
            toggleRButton.on("click", function(e){
                e.preventDefault();
                if ($(this).hasClass('hide-left-sidebar')) {
                    plugin.showleftSidebar();
                    $(this).removeClass('hide-left-sidebar');
                    store.set('leftSidebarToggle', 0);
                } else {                  
                    plugin.hideleftSidebar();
                    $(this).addClass('hide-left-sidebar');
                    store.set('leftSidebarToggle', 1);
                }
            });
        ...
        plugin.hideleftSidebar = function() {
            var breakpoint = plugin.getBreakPoint();
            $('#left-sidebar').addClass('hide-sidebar');
            $('.page-content, #footer').removeClass('left-sidebar-page');
            if (breakpoint == 'laptop' || breakpoint == 'tablet' || breakpoint == 'phone') {
                $('.page-content').removeClass('rOverLap');
            }
            $('#back-to-top').removeClass('leftsidebar');
        }

        //show left sidebar
        plugin.showleftSidebar = function() {
            var breakpoint = plugin.getBreakPoint();
            $('#left-sidebar').removeClass('hide-sidebar');
            if (breakpoint == 'laptop' || breakpoint == 'tablet' || breakpoint == 'phone') {
                $('.page-content').addClass('rOverLap');
            }
            $('.page-content, #footer').addClass('left-sidebar-page');
            $('#back-to-top').addClass('leftsidebar');
        }
        ...
        //untogle right sidebar
        plugin.unTogglerightSidebar = function () {
            var breakpoint = plugin.getBreakPoint();
            var scrollto = $("#sidebar .sidebar-scrollarea");
            if (plugin.settings.sidebar.fixed) {
                plugin.addScrollTo(scrollto,'left',plugin.settings.customScroll.color);
            }
            ...
            mainNavLinkAll.each(function(index) {
                ...
                if(href === url) {
                    ...
                    if(ulElem.hasClass('sub')) {
                        ...
                        if (_this.closest('ul').hasClass('sub')) {
                            var _ulElem =  _this.closest('ul');
                            _ulElem.addClass('show').css('display', 'block');
                            var __this = _(this).closest('li.hasSub').children('a.notExpand');
                            __this.removeClass('notExpand').addClass('expand active-state');
                            _this.closest('li.hasSub').addClass('highlight-menu');
                            _
                            if(plugin.settings.sideNav.showArrows) {
                                __this.find('.sideNav-arrow').removeClass('rotate0').addClass('rotate90');
                            }
                        }
                    } 
                } else {
        ...
        plugin.panels = function () {
            ...
            panels.each(function( index ) {
                ...
                //inject all controls per class in left side
                if(self.hasClass('toggle') || self.hasClass('panelClose') || self.hasClass('panelRefresh')) {
                    if(!panelHeading.find('.panel-controls-left').length) {
                        panelHeading.append('<div class="panel-controls panel-controls-left">');
                        panelControlsleft = panelHeading.find('.panel-controls-left');
                    } else {
                        panelControlsleft = panelHeading.find('.panel-controls-left');
                    }
                }
                ...
                //refresh
                if(self.hasClass('panelRefresh') && !panelControlsleft.find('a.panel-refresh').length) {
                    panelControlsleft.append('<a href="#" class="panel-refresh"><i class="'+ plugin.settings.panels.refreshIcon+'"></i></a>');
                }
                //Toggle
                if(self.hasClass('toggle') && !panelControlsleft.find('a.toggle').length) {
                    if (self.hasClass('panel-closed')) {
                        panelControlsleft.append('<a href="#" class="toggle panel-maximize"><i class="'+ plugin.settings.panels.collapseIcon+'"></i></a>');
                        self.find('.panel-body').slideToggle(0);
                        self.find('.panel-footer').slideToggle(0);
                        self.find('.panel-heading').toggleClass('min');
                    } else {
                        panelControlsleft.append('<a href="#" class="toggle panel-minimize"><i class="'+ plugin.settings.panels.toggleIcon+'"></i></a>');
                    }
                }
                //close
                if(self.hasClass('panelClose') && !panelControlsleft.find('a.panel-close').length) {
                    panelControlsleft.append('<a href="#" class="panel-close"><i class="'+ plugin.settings.panels.closeIcon+'"></i></a>');
                }
        ...
        plugin.storejs = function () {
            /* Copyleft (c) 2010-2013 Marcus Westin */
...
-1*delta}if(orgEvent.wheelDeltaY!==undefined)deltaY=orgEvent.wheelDeltaY/120;if(orgEvent.wheelDeltaX!==undefined)deltaX=-1*orgEvent.wheelDeltaX/120;args.unshift(event,delta,deltaX,deltaY);return($.event.dispatch||$.event.handle).apply(this,args)}})(jQuery);
        ...
        plugin.animatedProgressBars = function () {
            /*! bootstrap-progressbar v0.8.3 | Copyleft (c) 2012-2014 Stephan Groß | MIT license | http://www.minddust.com */
        ...
        plugin.chatApp = function () {
            ...
            var rbScroll = $('#left-sidebar .sidebar-scrollarea');
            ...
            users.click(function(){
                ...
                plugin.addScrollTo(scrollarea,'left',plugin.settings.customScroll.rscolor);
                if (plugin.settings.leftSidebar.fixed) {
                    //deactivate leftsidebar scroll
            ...
            chatback.click(function(){
                ...
                if (plugin.settings.leftSidebar.fixed) {
                   //add scroll to sidebar
                    plugin.addScrollTo(rbScroll, 'left', plugin.settings.customScroll.rscolor);
                }                
                //hide chat type
        ...
        plugin.removeDefaultClassess = function () {
            ...
            content.addClass('left-sidebar-page');
...
//                    plugin.showleftSidebar();
//                    store.set('leftSidebarToggle', 0);
//                  $('#toggle-left-sidebar').removeClass('hide-left-sidebar');
                    plugin.hideleftSidebar();
